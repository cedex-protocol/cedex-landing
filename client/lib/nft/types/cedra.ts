...
export const CEDRA_CONFIG = {
  ...
  indexer: getEnvVar("NEXT_PUBLIC_CEDRA_INDEXER"),
...
export function getCedraExplorerNFTURL(
  address: string,
  tokenId: string
): string {
  return `${CEDRA_NETWORK_INFO.explorer}/account/${address}?tab=Tokens#${tokenId}`;
}
